...
class SubDLProvider(SubtitleProviderBase):
  ...
  def search(self, context, language_code, max_results):
    ...
    for candidate in candidates:
      ...
      mapped = self._normalize_subtitles(subtitles, language_code)
      for item in mapped:
  ...
  def _normalize_subtitles(self, subtitles, language_code):
    ...
    for item in subtitles:
      ...
      release_name = _as_text(item.get('release_name') or item.get('name') or 'subtitle').strip()
      if not release_name:
...
def _strip_year_token(value):
  ...
  if len(tokens) == 0:
    return _as_text(value).strip()
  stripped = [token for token in tokens if not re.match(r'^(19\d{2}|20\d{2})$', token)]
  if len(stripped) == 0:
    stripped = tokens
...
def _extract_subtitle_bytes(raw_data):
  if raw_data is None or len(raw_data) == 0:
    raise ProviderRequestError('SubDL download payload is empty.')
  ...
  if raw_data[:2] == b'\x1f\x8b':
    try:
      ...
      raise ProviderRequestError('SubDL gzip extraction failed: %s' % (exc))
    ...
    try:
      with zipfile.ZipFile(io.BytesIO(raw_data)) as zip_file:
        ...
        if not candidate_name:
          raise ProviderRequestError('SubDL zip does not contain an .srt file.')
        return zip_file.read(candidate_name)
    except ProviderRequestError:
      raise
    except Exception as exc:
      raise ProviderRequestError('SubDL zip extraction failed: %s' % (exc))
