...
def _extract_subtitle_bytes(raw_data):
  if raw_data is None or len(raw_data) == 0:
    raise ProviderRequestError('BSPlayer download payload is empty.')
  ...
  if raw_data[:2] == b'\x1f\x8b':
    try:
      ...
      raise ProviderRequestError('BSPlayer gzip extraction failed: %s' % (exc))
    ...
    try:
      with zipfile.ZipFile(io.BytesIO(raw_data)) as zip_file:
        ...
        if not candidate_name:
          raise ProviderRequestError('BSPlayer zip does not contain an .srt file.')
        return zip_file.read(candidate_name)
    except ProviderRequestError:
      raise
    except Exception as exc:
      raise ProviderRequestError('BSPlayer zip extraction failed: %s' % (exc))
